...
then
  ...
  NODE1_IP="regen.spanish-node.es"
  RPC2="http://$NODE2_IP"
  RPC_PORT2=6767
  P2P_PORT2=6768

  NODE2_IP="public-rpc.regen.vitwit.com"
  RPC1="http://$NODE1_IP"
  P2P_PORT1=26656
  RPC_PORT1=26657
  ...
  BLOCK_HEIGHT=$((($(($LATEST_HEIGHT / $INTERVAL)) - 1000) * $INTERVAL)); #Mark addition
  ...
  echo ""
  echo "##################################################################"
  echo "#     Parameters to change in: .regen/config/config.toml          #"
  echo "##################################################################"
